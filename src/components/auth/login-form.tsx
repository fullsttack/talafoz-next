...
import { sendOTP, verifyOTP, socialLogin } from "@/app/actions/auth";
...
export function LoginForm() {
  ...
  const handleSocialLogin = async (provider: "google" | "github") => {
    try {
      ...
      const url =
        provider === "google"
          ? `https://accounts.google.com/o/oauth2/v2/auth?client_id=${process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID}&redirect_uri=${process.env.NEXT_PUBLIC_APP_URL}/api/auth/callback/${provider}&response_type=code&scope=email profile`
          : `https://github.com/login/oauth/authorize?client_id=${process.env.NEXT_PUBLIC_GITHUB_CLIENT_ID}&redirect_uri=${process.env.NEXT_PUBLIC_APP_URL}/api/auth/callback/${provider}&scope=user:email`;
