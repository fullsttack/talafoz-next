...
export default function CourseCart({ course, gradientColor }: CourseCartProps) {
  // Function to safely extract price and apply discount
  ...
  const getDiscountedPrice = (price: string, discountPercent: number) => {
    ...
    // If we can't parse or calculate the discount, return the original price
    return price;
  };
  ...
  return (
    <Link href={`/courses/${course.slug}`} className="block">
      <Card
        className="group overflow-hidden border-0 shadow-md hover:shadow-cyan-500/50 hover:shadow-lg transition-all duration-300 cursor-pointer"
      >
        <DynamicMagicCard
        ...
        >
          ...
          <CardFooter className="pt-2 flex justify-center items-center">
            {course.discount > 0 ? (
              <div className="flex flex-col items-center">
                ...
                <div className="flex items-center gap-2">
                  <div className="text-primary font-bold text-lg">
                    {formatPriceDisplay(getDiscountedPrice(course.price, course.discount))}
                  </div>
